...
export const Dialog: React.FC<types.DialogType> = ({
  ...
  return (
    <React.Fragment>
      <Modal isOpen={isOpen} toggle={toggleDialog} className={className} ref={innerRef}>
        <ModalHeader toggle={toggleDialog} className={headerClassName}>
          {
            ...
            type === 'info' ? <FontAwesomeIcon icon={fa.faInfoCircle} color="#6B7376" size="lg" /> :
        ...
        { !hideCloseButton && <Button onClick={() => onclose()} color={closeButtonClassName}>{closeText}</Button> }
